 ...
 **/
class RadioTest extends RowWithChoicesTestCase
{

    protected function getRow($name, $label = null, $attributes = array())
    {
        return new Radio($name, $label, $attributes);
    }

    public function testInput()
    {
        $r = $this->getRow('decision');
        $html = Html::input('radio', 'decision', array());
    ...
    public function testInputWithChoices()
    {
        ...
        $html = Html::input('radio', 'decision', array('Yes' => 'yes', 'No' => 'no'));
        $this->assertSame($html, $r->input());
    }
    ...
    public function testInputWithValueAndChoices()
    {
        ...
        $r->setChoices(array('Yes' => 'yes', 'No' => 'no'));
        $r->setValue('no');
        $html = Html::input('radio', 'decision', array('Yes' => 'yes', 'No' => 'no'), 'no');
        $this->assertSame($html, $r->input());
    }
    ...
    public function testInputWithStrangeTypes()
    {
        ...
        $html = Html::input('radio', 'decision', $choices);
        $this->assertSame($html, $r->input());
    }
    ...
    public function testRow()
    {
        ...
        $expected .= Html::input('radio', 'decision', array('Yes' => 'yes', 'No' => 'no'));
    ...
    public function testRowWithValue()
    {
        ...
        $r->setChoices(array('yes', 'no'));
        $expected = Html::label('decision', 'Decision');
        $expected .= Html::input('radio', 'decision', array('Yes' => 'yes', 'No' => 'no'), 'yes');
        $this->assertSame($expected, $r->render());
    }
    ...
    public function testRowWithError()
    {
        ...
        $r->setError($error);
        $expected = Html::label('decision', 'Decision');
        $expected .= Html::input('radio', 'decision', array());
    ...
    public function testRowWithValueAndError()
    {
        ...
        $r->setChoices(array('yes', 'no'));
        $expected = Html::label('decision', 'Decision');
        $expected .= Html::input('radio', 'decision', array('Yes' => 'yes', 'No' => 'no'), 'no');
        $expected .= '<small class="error">' . $error . '</small>';
        $this->assertSame($expected, $r->render());
    }
